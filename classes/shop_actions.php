...
class Shop_Actions extends Cms_ActionScope
{
    ...
    public function checkout()
    {
        ...
        {
            $noShippingOption = Shop_ShippingOption::find_by_api_code('no_shipping_required');
            if ($noShippingOption)
            {
                Shop_CheckoutData::setSelectedShippingOption($noShippingOption, post('cart_name', 'main'));
        ...
        } else if($selectedShippingQuote){
            $selectedShippingOption = $selectedShippingQuote->getShippingOption();
            $noShippingOption = Shop_ShippingOption::find_by_api_code('no_shipping_required');
            if ($noShippingOption && ($selectedShippingQuote->id == $noShippingOption->id)){
                Shop_CheckoutData::resetSelectedShippingQuote();
            }
        }
            ...
            switch ($checkout_step)
            {
                ...
                case 'shipping_method' :
                    if (!$skip_shipping_step) {
                        Shop_CheckoutData::setSelectedShippingQuote(null, post('cart_name', 'main'));
                    }
                    break;
        ...
        $this->data['selected_shipping_quote'] = Shop_CheckoutData::getSelectedShippingQuote();
        ...
        elseif ($checkout_step == 'shipping_info')
        {
            ...
            if($selectedShippingQuote){
                $shipping_quote = $selectedShippingQuote->isFree() ? 0 : $selectedShippingQuote->getPrice();
            }
            $this->data['shipping_quote'] = $shipping_quote;
        }
        elseif ($checkout_step == 'shipping_method')
        {
            ...
            $this->data['shipping_quote'] = 0;
            $this->data['shipping_taxes'] = array();
            $this->data['shipping_tax'] = 0;
            $selectedShippingQuote = Shop_CheckoutData::getSelectedShippingQuote();
            if($selectedShippingQuote){
                $total += $this->data['shipping_quote'] = $selectedShippingQuote->isFree() ? 0 : $selectedShippingQuote->getPrice();
                $shipping_taxes = $this->data['shipping_taxes'] = Shop_CheckoutData::getShippingTaxes($cart_name, $selectedShippingQuote);
        ...
        $selectedShippingQuote = Shop_CheckoutData::getSelectedShippingQuote();
        $this->data['selected_shipping_quote'] = $selectedShippingQuote;
    ...
    protected function load_checkout_estimated_data($cart_name)
    {
        ...
        if ($display_prices_including_tax && isset($this->data['selected_shipping_quote']))
        {
            $shippingQuote = $this->data['selected_shipping_quote'];
            $shippingQuote->setTaxInclMode(false);
            $shippingQuote->quote_tax_incl = $shippingQuote->getPrice() + $shipping_tax;
            $shippingQuote->setTaxInclMode(true);
        }
    ...
    public function checkout_payment_method()
    {
        ...
        $this->data['shipping_tax'] = 0;
        if($selectedShippingQuote) {
            $total += $this->data['shipping_quote'] = $selectedShippingQuote->getPrice();
            $shipping_taxes = $this->data['shipping_taxes'] = Shop_CheckoutData::getShippingTaxes($cart_name, $selectedShippingQuote);
            $total += $this->data['shipping_tax'] = Shop_TaxClass::eval_total_tax($shipping_taxes);
        }
    ...
    public function checkout_order_review()
    {
        ...
        $this->data['shipping_method'] = Shop_CheckoutData::get_shipping_method(); //@deprecated
        $this->data['selected_shipping_quote'] = Shop_CheckoutData::getSelectedShippingQuote();
