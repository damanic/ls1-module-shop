<?
	...
	class Shop_CartProductFixedPrice_Action extends Shop_CartRuleActionBase
	{
		...
		{
			$host_obj->add_field('discount_amount', 'New cart item price', 'full', db_float)->comment('Please specify a fixed price for the cart item.', 'above')->validation()->required('Please specify discount amount');
		}
		...
		public function eval_discount(&$params, $host_obj, &$item_discount_map, &$item_discount_tax_incl_map, $product_conditions)
		{
			if (!array_key_exists('cart_items', $params))
				throw new Phpr_ApplicationException('Error applying the "Discount each cart item unit price to a fixed price" price rule action: the cart_items element is not found in the action parameters.');
			...
			foreach ($cart_items as $item)
			{
				...
				if ($item_current_price === false)
					$item_current_price = $item->set_de_cache_item('current_price', $item->single_price_no_tax(false) - $item->get_sale_reduction());
