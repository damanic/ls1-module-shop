...
class Shop_Stock_Report extends Shop_GenericReport
{
    ...
    public function listPrepareData()
    {
        ...
        $obj->calculated_columns['items_ordered'] = array(
            'sql' => "sum(shop_order_items.quantity)",
            'type' => db_number
        );
        $obj->calculated_columns['stock_sku'] = array(
            'sql' => "COALESCE(shop_option_matrix_records.sku, shop_products.sku)",
            'type' => db_varchar
        );
        $obj->calculated_columns['stock_in_stock'] = array(
        ...
        $obj->join('shop_option_matrix_records',
    ...
    public function listExtendModelObject($model)
    {
        if ($model instanceof Shop_Product) {
            ...
            $model->calculated_columns['stock_sku'] = array(
                'sql' => "COALESCE(shop_option_matrix_records.sku, shop_products.sku)",
                'type' => db_varchar
            );

            $model->define_column('stock_name', 'Name')->invisible();
            $model->calculated_columns['stock_name'] = array(
                'sql' => $this->get_stock_name_sql(true),
                'type' => db_varchar
            );

            $model->define_column('stock_in_stock', 'Units In Stock')->invisible();
            $model->calculated_columns['stock_in_stock'] = array(
                'sql' => "COALESCE(shop_option_matrix_records.in_stock, shop_products.in_stock)",
                'type' => db_number
            );
        }
