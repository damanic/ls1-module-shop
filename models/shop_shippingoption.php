...
class Shop_ShippingOption extends Shop_ActiveRecord
{
    ...
    public $has_and_belongs_to_many = array(
        'countries' => array(
            'class_name' => 'Shop_Country',
            'join_table' => 'shop_shippingoptions_countries',
            'order' => 'name'
        ),
        'customer_groups' => array(
            'class_name' => 'Shop_CustomerGroup',
    ...
    public function define_columns($context = null)
    {
        $this->define_column('name',
            'Name')->order('asc')->validation()->fn('trim')->required('Please specify the shipping option name.');
        ...
        $this->define_column('ls_api_code',
            'LemonStand API Code')->defaultInvisible()->validation()->fn('trim')->fn('mb_strtolower')->unique('Shipping option with the specified LemonStand API code already exists.');
        ...
        if (!$front_end) {
            ...
            $this->define_multi_relation_column('customer_groups', 'customer_groups', 'Customer Groups',
                '@name')->defaultInvisible();
        }
    ...
    public function define_form_fields($context = null)
    {
        ...
        if ($context != 'print_label') {
            $this->add_form_field('enabled',
                'left')->tab('General Parameters')->comment('Make this shipping option available on the front-end website.');
            $this->add_form_field('taxable',
                'right')->tab('General Parameters')->comment('Use tax class "Shipping" to specify tax rates for different locations.');
            $backend_enabled = $this->add_form_field('backend_enabled',
                'left')->tab('General Parameters')->comment('Make this shipping option available in the Administration area.');
            if ($this->enabled) {
                $backend_enabled->disabled();
            }

            $this->add_form_field('name')->comment('Name of the shipping option. It will be displayed on the front-end website.',
                'above')->tab('General Parameters');
            $this->add_form_field('description')->comment('If provided, it will be displayed on the front-end website.',
                'above')->tab('General Parameters')->size('small');

            $this->add_form_field('handling_fee',
                'left')->tab('General Parameters')->comment('Please specify a handling fee for this shipping method. The handling fee will be added to the shipping quote.',
                'above');
            $this->add_form_field('ls_api_code',
                'right')->comment('You can use the API Code for identifying the shipping method in the API calls.',
                'above')->tab('General Parameters');

            $this->add_form_field('min_weight_allowed',
                'left')->comment('The shipping option will be ignored if the package weight is less than the specified value. Leave the field empty to cancel the minimum weight check.',
                'above')->tab('General Parameters');
            $this->add_form_field('max_weight_allowed',
                'right')->comment('The shipping option will be ignored if the package weight is more than the specified value. Leave the field empty to cancel the maximum weight check.',
                'above')->tab('General Parameters');

            if (!$front_end) {
                $this->add_form_field('customer_groups')->tab('Visibility')->comment('Please select customer groups the shipping options should be available for. If no groups are selected, the shipping option will be available for all customer groups.',
                    'above');
            }
            ...
            if ($obj->config_countries() && !$front_end) {
                $this->add_form_field('countries')->tab('Countries')->comment('Countries the shipping method is applicable to. Uncheck all countries to make the shipping method applicable to any country.',
                    'above')->referenceSort('name');
            }
    ...
    public function field_error($field, $message, $grid_row = null, $grid_column = null)
    {
        if ($grid_row != null) {
            ...
            $this->validation->setWidgetData(Db_GridWidget::get_cell_error_data($this, $field, $grid_column,
                $grid_row));
        }
    ...
    public function add_field($code, $title, $side = 'full', $type = db_text)
    {
        ...
        $form_field = $this->add_form_field($code,
            $side)->tab('Configuration')->optionsMethod('get_added_field_options')->optionStateMethod('get_added_field_option_state');
    ...
    public function getQuoteForCheckout($cart_name = 'main')
    {
        ...
        if ($quotes === null) {
            ...
            if ($rates === null) {
                try {
                    $shippingProvider->setEventParameters($eventParams); //legacy support
                    $rates = $shippingProvider->getItemRates($this, $cartItems, $shippingInfo, null,
                        'getQuoteForCheckout');
    ...
    public static function is_taxable($id)
    {
        ...
        return self::$is_taxable_cache[$id] = Db_DbHelper::scalar('select taxable from shop_shipping_options where id=:id',
            array('id' => $id));
    }
    ...
    public static function getShippingOptionsForOrder($order, $frontendOnly = false, $includeDisabled = false)
    {
        ...
        if ($shippingOptionsArray) {
            ...
            $shippingOptionsArray = self::applyFilterCountry($shippingOptionsArray,
    ...
    protected function buildQuotes(array $rates, array $eventParams)
    {
        ...
        foreach ($active_items as $item) {
            ...
            if ($product) {
                $total_per_product_cost += $product->get_shipping_cost($shippingInfo->country, $shippingInfo->state,
                        $shippingInfo->zip) * $item->quantity;
            }
        }
        ...
        foreach ($rates as $rateInfo) {
            ...
            if ($eventParams['display_prices_including_tax']) {
                ...
                $shipping_taxes = Shop_TaxClass::get_shipping_tax_rates($this->id, $shippingInfo,
                    $quoteInfo->getPrice());
     ...
     * @return mixed
     */
    private static function findByWeight(
        $weight,
        $frontendOnly = false,
    ...
    private static function applyLegacyFilterEvents($shippingOptions, $eventParams = array())
    {
        ...
        $shippingOptions = Backend::$events->fire_event(array(
            'name' => 'shop:onUpdateShippingOptions',
            'type' => 'update_result'
        ), $shippingOptions, $eventParams);
     ...
     *
     * Params can be updated via event @param Shop_ShippingOption $shipping_option
    ...
    protected function fetch_quote($params)
    {
        ...
        if ($shipping_type->config_countries()) {
            $country_ids = Db_DbHelper::scalarArray('select shop_country_id from shop_shippingoptions_countries where shop_shipping_option_id=:id',
                array('id' => $this->id));
            ...
            foreach ($result as $name => &$option_obj) {
                ...
                if (isset($option_obj['currency']) && strlen($option_obj['currency']) == 3) {
                    if ($option_obj['currency'] !== $shop_currency_code) { //convert to shop currency
                        $option_obj['quote'] = $currency_converter->convert($option_obj['quote'],
                            $option_obj['currency'], $shop_currency_code);
                    }
                }
    ...
    protected static function filter_applicable_options($shipping_options, $params)
    {
        ...
        foreach ($shipping_options as $option) {
            ...
            $apply_customer_group_filter = strlen($params['customer_group_id']);
            if ($apply_customer_group_filter && !self::option_visible_for_customer_group($option->id,
    ...
    protected function apply_quote($quote, $params)
    {
        ...
        foreach ($active_items as $item) {
            $product = $item->product;
            if ($product) {
                $total_per_product_cost += $product->get_shipping_cost($params['country_id'], $params['state_id'],
                        $params['zip']) * $item->quantity;
            }
        }
        ...
        foreach ($quotes as $index => $q) {
            ...
            $shipping_taxes = Shop_TaxClass::get_shipping_tax_rates($this->id, $params['shipping_info'],
                $quoteObj->quote);
            $shippingTax = Shop_TaxClass::eval_total_tax($shipping_taxes);
